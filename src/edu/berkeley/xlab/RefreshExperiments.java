...
public class RefreshExperiments extends AsyncTask<Void, Void, Void> {
	...
	public RefreshExperiments(Context context, Activity activity,
			boolean downloadFlag, ExperimentAbstract toBeRemoved) {
	...
	public RefreshExperiments(Context context, boolean downloadFlag,
			ExperimentAbstract toBeRemoved) {
	 ...
	 *            True if interacts with UI, false otherwise
	 */
	private void initialize(Context context, boolean downloadFlag,
	 ...
	 */
	private void initialize(Context context, boolean downloadFlag,
			ExperimentAbstract toBeRemoved, boolean showMessages) {
		this.context = context;
		this.downloadFlag = downloadFlag;
	...
	protected void onPreExecute() {
		...
		this.username = Utils.getStringPreference(context, Constants.USERNAME,
				"anonymous");
	...
	protected Void doInBackground(Void... whatever) {
		...
		responsesList = context.getSharedPreferences(
				ResponseAbstract.RESPONSES_LIST, Context.MODE_PRIVATE);

		Log.d(TAG,
				"Response SharedPreferences names: "
						+ responsesList.getString("SharedPreferences", ""));
	...
	private void finishUp() {
		...
		XLabSuperObject.setList(context, ResponseAbstract.RESPONSES_LIST,
				responseNamesArrayList);
		...
		if (showMessages) {
			...
			UIHandler uiHandler = new UIHandler(
					((HandlerThread) uiThread).getLooper());
	...
	public class UploadSingleResponse implements Runnable {
		...
		public void run() {
			...
			try {
				...
				case Constants.XLAB_TQ_EXP:
					Log.d(TAG,
							Constants.XLAB_API_ENDPOINTS[Constants.XLAB_TQ_EXP]
					...
					http_response = Utils
							.getData(Constants.XLAB_API_ENDPOINTS[0]
									+ "?tq_id=" + expId + "&tq_response="
									+ URLEncoder.encode(answer, "utf-8")
									+ "&tq_username=" + username + "&tq_lat="
									+ BackgroundService.getLastLat()
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							case Constants.XLAB_BL_EXP:
								...
								if (Utils.checkIfSaved(context,
										...
										ExperimentAbstract.EXP_LIST)) {
									Log.d(TAG,
											"About to create bl object from JSON from SP");
									SharedPreferences sharedPreferences = context
											.getSharedPreferences(
													ExperimentAbstract.makeSPName(json
															.getInt("id")),
													Context.MODE_PRIVATE);
									bl = new ExperimentBudgetLine(context,
											sharedPreferences);
									...
									case Constants.TIMER_STATUS_RESTRICTIVE:
										...
										case Constants.TIMER_DYNAMIC:
											new TimerDynamic(context, activity,
													bl, sharedPreferences);
											break;
